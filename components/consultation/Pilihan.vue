  ...
  <ClientOnly>
    <div class="sm:px-4 xl:px-20">
      <div
        ...
        <img class="absolute bottom-0 left-0 w-full z-0" src="/image/landing-page/shape-wave-advokat.webp" alt="" />
        <div class="md:py-2 space-y-2 relative">
          <h2 class="md:text-3xl font-semibold text-xl text-black">
            Pilihan Advokat
          </h2>
          <p class="text-sm md:text-xs lg:text-base font-dm-sans font-normal antialiased text-neutral-900 mb-6">
            Advokat online untuk kebutuhan hukum Anda. Ingin mengajukan layanan
            <span class="text-transparent bg-gradient-to-r from-blue-2 to-blue-3 bg-clip-text font-bold">Pro
              bono</span>? Klik di
        ...
        <div class="relative">
          <Swiper :slides-per-view="isMobile ? 1 : 2" :space-between="50" :pagination="pagination"
            :modules="[Pagination, Autoplay, Navigation]" @swiper="onSwiper" @slide-change="onSlideChange">
            <SwiperSlide v-for="item in items" :key="item.id">
              <div class="bg-white relative rounded-lg min-w-[332px] max-h-[185px] my-2 cursor-pointer">
                <!---->
                <div class="relative py-2">
                  <div class="absolute left-2 top-2">
                    <span class="relative z-[5] flex h-3 w-3"><span
                        class="animate-ping-slow absolute inline-flex h-full w-full rounded-full opacity-75 bg-green-200"></span><span
                        class="relative inline-flex rounded-full h-3 w-3 bg-green-500"></span></span>
                  </div>
                  <div class="px-2 flex gap-2">
                    <div class="relative h-fit">
                      <img src="https://storage.googleapis.com/perqara-public/lawyer/photo_1722932861"
                        class="min-w-[62px] w-[62px] min-h-[93px] h-[93px] rounded-md object-cover" />
                      <div v-if="item.video" id="tagging-video-voice-call"
                        ...
                        <NuxtImg alt="video-icon" class="w-2" src="/image/landing-page/video-icon.png" />
                        <p class="text-[8px] text-center text-white font-semibold antialiased">
                          Tersedia
                        </p>
                      </div>
                    </div>
                    <div class="flex-1 flex flex-col gap-1">
                      <div class="flex items-center h-[40px]">
                        <span class="text-base font-bold line-clamp-2 leading-5 text-black">{{ item.name }}</span>
                      </div>
                      <div class="flex gap-1 text-xs items-center">
                        <div class="flex gap-1 items-center">
                          <NuxtImg alt="location" class="w-4 h-4" src="/image/landing-page/icon-location.png" /><span
                            class="text-[#4D5B6A] capitalize">{{
                              item.location
                            }}</span>
                        </div>
                        <div class="border-l h-2/3 border-gray-500"></div>
                        <div class="flex gap-1 flex-1">
                          <NuxtImg class="w-4 h-4" src="/image/landing-page/icon-medal-star.png" alt="medal-star" />
                          <span class="text-[#0072C3] truncate">Terverifikasi oleh Peradi</span>
                        </div>
                      </div>
                      <div class="flex items-center">
                        <p class="text-xs antialiased text-[#344052] my-auto flex gap-1 items-center">
                        ...
                        <span class="text-xs text-[#757E8C] ml-1">({{ item.count }} Konsultasi)</span>
                      </div>
                  ...
                  <div class="flex justify-between items-center gap-1 px-2">
                    <div class="flex items-center gap-0.5">
                      ...
                      <div class="flex flex-col gap-0.5 md:gap-1 rounded-lg">
                        ...
                        <div class="flex gap-1 items-center">
                          <p class="text-blue-2 font-semibold !leading-5 text-sm">
                            Rp{{ item.price }}
                          </p>
                    ...
                    <button class="font-medium px-4 py-2.5 rounded-md text-xs bg-blue-2 text-white">
                      Konsultasi
                    </button>
                  </div>
                </div>
          ...
          <div class="flex justify-center sm:justify-between mt-4">
            ...
            <div class="flex">
              <button class="navigation-prev-lawyer button-arrow mr-3" :disabled="activeIndex === 0"
                :class="{ 'opacity-50': activeIndex === 0 }" @click="swiperRef.slidePrev()">
                <img class="rotate-180" src="/image/arrow-right.svg" alt="" />
              </button>
              <button class="navigation-next-lawyer button-arrow" :class="{ 'opacity-50': isEnd }" :disabled="isEnd"
